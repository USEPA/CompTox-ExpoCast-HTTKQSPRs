...
```
```{r sanity_test}
humres <- calc_tkstats(dtxsid="DTXSID8023719",species="human",restrictive.clearance=TRUE)
humnonres <- calc_tkstats(dtxsid="DTXSID8023719",species="human",restrictive.clearance=FALSE)
ratres <- calc_tkstats(dtxsid="DTXSID8023719",species="rat",restrictive.clearance=TRUE)
ratnonres <- calc_tkstats(dtxsid="DTXSID8023719",species="rat",restrictive.clearance=FALSE)
...
tidy_summary %>%
  dplyr::mutate(SLE = (log10(1e-6+hclint) - log10(1e-6+rclint))^2) %>% 
...
```{r make_css_plot}
plotCss <- ggplot(rat_df, aes(y = 10^-3 + Rat_Css_NR, 
                              x = 10^-3 + RatHum_Css_NR)) +
  geom_point() +
  geom_abline() +
  annotate(
    label = paste0("RMSLE\n = ", 
...
```{r make_cmax_plot}
plotCmax <- ggplot(rat_df, aes(y = 10^-3 + Rat_Cmax_NR, 
                               x = 10^-3 + RatHum_Cmax_NR)) +
  geom_point() +
  geom_abline() +
  annotate(
    label = paste0("RMSLE\n = ", 
...
```{r make_auc_plot}
plotAUC <- ggplot(rat_df, aes(y = 10^-3 + Rat_AUC_NR, 
                              x = 10^-3 + RatHum_AUC_NR)) +
  geom_point() +
  geom_abline() +
  annotate(
    label = paste0("RMSLE\n = ", 
...
cowplot::plot_grid(
  plotlist = list(plotCss, plotCmax, plotAUC),
  labels = c("A", "B", "C"),
  ncol=1
)
...
write.csv(rathum.table,
          file="tables/SupTab-Interspecies.txt")
```
```{r textforpaper}
paste("There were",
