...
CVTDTXSID.COL <- "analyte_dtxsid"
CVTCAS.COL <- "analyte_casrn"
CVTREFERENCE.COL <- "pmid"

series_res_set <- read.csv("CvTdb/CvTdb_fullData_2025April.csv")
...
# No records for this in CvTdb anymore
...
cat("Number of unique CAS with CvT data:")
length(eval.cas)
...
cat("Number of unique DTXSIDs with CvT data:")
length(CvT.chems)
...
cat("Number of unique DTXSIDs in the test set with CvT data")
length(unique(
...
fittable <- as.data.frame(read_excel(
  "CvTdb/evalTKstats_bakeoff_2025April.xlsx",
  sheet=1))
...
write.csv(fittable.all,file="tables/SupTable-TKFits.txt",row.names=FALSE)
...
cat(paste("Among the",
          ...
          "in human.\n"))
unique(subset(CvT.data, CAS %in% CvT.chems$CASRN & Species=="human")$Compound)  


cat(paste(dim(subset(CvT.chems, CASRN %in% pharma$CASRN))[1],
          "of the evaluation chemicals were pharmaceuticals.\n"))

cat(paste("The evaluation chemicals included:",
            dim(subset(CvT.chems, CASRN %in% TSCA$CASRN))[1],
          "from the non-confidential Toxic Substances Control Act (TSCA) active inventory,",
            dim(subset(CvT.chems, CASRN %in% pest.active$CASRN))[1],
          "pesticide active ingredients,",
          dim(subset(CvT.chems, CASRN %in% consumer$CASRN))[1],
          "that are found in consumer products,",
          ...
          "that are part of the ToxCast screening program.\n"))
