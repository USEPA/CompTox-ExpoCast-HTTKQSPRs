---
title: "HTTK QSPRs: Interspecies Evaluation"
...
tidy_summary %>%
  dplyr::mutate(SE = (log10(hfup+1e-3) - log10(rfup+1e-3))^2) %>% 
  dplyr::ungroup() %>% dplyr::pull(SE) %>%
  {sqrt(sum(.)/length(.))} -> fup_rmsle
```
...
tidy_summary %>%
  dplyr::mutate(SLE = (log10(hclint+1e-3) - log10(rclint+1e-3))^2) %>% 
