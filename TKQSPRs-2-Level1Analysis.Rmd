...
```{r load_known_traning_sets, eval=TRUE}
...
for (this.row in 1:dim(level1tab)[1])
{
  ...
  if (!is.na(clint))
  {
    if (clint < TOOCLOSE.THRESHOLD)
    {
      ...
      training.set.table[training.set.table$DTXSID==this.dtxsid,
                         paste("Human.Clint.",this.qspr,sep="")] <- 
                        "Possible"
...
cat(paste("A total of",
    ...
    "QSPR predictions were removed as known or suspected training set data (see Supplement Materials Table SupTable-TrainingSetChems.txt).\n"))
cat("\n")
cat(paste("A total of",
          dim(subset(training.set.table,"Human.Fup.OPERA"!="" |
                       "Human.Clint.OPERA" != ""))[1],
          "predictions were removed from OPERA for possibly being in the training set (see Supplement Materials Table SupTable-TrainingSetChems.txt).\n"))
cat("\n")
...
save(training.set.table, file="data/trainingchems.RData")
