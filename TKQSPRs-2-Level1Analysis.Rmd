...
cat(paste(sum(invitro$Human.Clint%in% 0),
          ...
          "chemicals with CvT data have no measured in vitro clearance compared with",
          tot.zero,
          "out of",
          tot.clint,
...
for (this.col in seq(13,21,2)) CvT.chems.scale[,this.col] <- 
  ...
  heatmap.2(as.matrix(CvT.chems.scale[,4:(dim(CvT.chems.scale)[2])]),
    ...
    trace="none",
    col=mypalette)
  png(paste("figs/Fig1-physchemandpredictors-heatmap-",
           output.prefix,
           ".tiff",sep=""),
...
kable(pred.counts)
write.csv(pred.counts,row.names = FALSE,
...
print(FigLev1a)
ggsave(paste("figs/Fig2-Lev1a-Clint-obsvspred",output.prefix,".tiff",sep=""), 
...
FigLev1c <- ggplot(data=subset(level1tab,!(QSPR %in% "IVBP"))) +
  ...
  theme( text  = element_text(size=20))
print(FigLev1c)
ggsave(paste("figs/Fig3-Lev1c-Fup-obsvpred-",output.prefix,".tiff",sep=""), 
       width=6, height=4, dpi=300)
```
