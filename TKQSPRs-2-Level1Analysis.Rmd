...
for (this.row in 1:dim(level1tab)[1])
{
  ...
  if (!is.na(clint))
  {
    if (clint < TOOCLOSE.THRESHOLD)
    {
      ...
      level1tab[this.row,"Human.Clint.AbsFE"] <-NA
      level1tab[this.row,"Human.Clint.FE"] <-NA
      print(paste("Human.Clint.",this.qspr,sep=""))
  ...
  if (!is.na(fup))
  {
    if (fup < TOOCLOSE.THRESHOLD)
    {
      ...
      level1tab[this.row,"Human.fup.AbsFE"] <-NA
      level1tab[this.row,"Human.fup.FE"] <-NA
      print(paste("Human.Fup.",this.qspr,sep=""))
      direct.retrieval.table <- rbind(direct.retrieval.table,
        data.frame(QSPR=this.qspr,
                   DTXSID = this.dtxsid,
                   Property = "Fup",
...
write.csv(direct.retrieval.table, row.names=FALSE,
          file="tables/SupTable-PossibleDirectRetrievals.txt")
```
...
training.set.table <- CvT.chems[,c(1,16:24)]
training.set.table[,2:10] <- ""
...
write.csv(train.set.sum.table, row.names=FALSE,
          file="tables/Table-TrainingSetSummary.txt")
...
cat(paste("A total of",
    ...
    "QSPR predictions were removed from the Level 1 evaluation since they were training set data (see Supplement Materials Table SupTable-TrainingSetChems.txt).\n"))
cat("\n")
cat("Table X summarizes the training sets for each QSPR approach and predicted endpoint. Training sets were evaluated for relative fraction of pharmaceutical chemicals and number of CvT evaluation chemicals included.\n")
cat("\n")
cat("In Table X we indicate the number of evaluation chemicals with predictions inside each QSPRs domain of applicability. In table X we also identify how many of the evaluation chemicals were included in the various training sets. Since many of the evaluation chemicals were in the various training sets, we still included QSPR predictions in the CvT evaluations when they not appear to be direct retrievals of a training set value (as in a nearest-neighbor algorithm identifying the compound itself). We believe that there is value to evaluating the predictions made as long as the method of prediction is the same as what would be used for a chemical without a measured value.")
cat("\n")
...
kable(ks.tests.fup)
write.csv(FE.table, file="tables/Table6-Level1FoldError.txt")
...
ggsave(paste("figs/Fig4-Lev1c-Fup-obsvpred-twopanel",output.prefix,".tiff",sep=""), 
