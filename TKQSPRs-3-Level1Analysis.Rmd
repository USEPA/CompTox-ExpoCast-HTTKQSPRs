...
write.table(training.set.table, 
            sep="\t",
            row.names=FALSE,
...
# IFS-QSAR (same as Papa 2018):
train.set.sum.table[train.set.sum.table$QSPR=="IFS-QSAR","Num.Chems"] <-
  dim(papa.hl)[1]
train.set.sum.table[train.set.sum.table$QSPR=="IFS-QSAR","Fraction.Pharma"] <-
  signif(sum(papa.hl$DTXSID %in% pharma$DTXSID)/dim(papa.hl)[1], 2)
train.set.sum.table[train.set.sum.table$QSPR=="IFS-QSAR","Num.Eval.Chems"] <-
  sum(training.set.table$"Human.HL.QSAR-INS" == "Training Set")
# IVBP:
...
write.table(train.set.sum.table, 
            row.names=FALSE,
            sep="\t",
...
write.table(direct.retrieval.table, 
            row.names=FALSE,
            sep="\t",
          file=paste("tables/SupTable",
          PREV.SUP.TABLES + 5,
...
{
  CvT.chems.scale[,this.col] <- (CvT.chems.scale[,this.col] -
    mean(CvT.chems.scale[,this.col],na.rm=TRUE)) / 
    sd(CvT.chems.scale[,this.col],na.rm=TRUE)
}
# HTTK Cols:
# Log transform:
CvT.chems.scale[,httk.start:last.col] <- 
  log10(CvT.chems.scale[,httk.start:last.col]+1e-3)
# Scale and center
...
for (this.qspr1 in unique(as.character(level1tab$QSPR)))
{
  ...
  FE.table[this.qspr1,"Clint.RMSLE"] <- 
    calc_RMSLE(this.subset,
               "Human.Clint.httk",
               "Human.Clint.pred")
...
write.table(Human.Clint.stats.table, 
            sep="\t",
            file="tables/SupTable-ClintStats.txt")
write.table(Human.fup.stats.table, 
            sep="\t",
            file="tables/SupTable-FupStats.txt")
```
