...
ComplexHeatmap::row_dend(ht) -> dendrow
...
ccd_lists <- read_csv(paste0(Sys.getenv("OD_DIR"),
...
png("figs/CCD_lists_heatmap_Lvl4.png",
    ...
    height = 8,
    width = 14, res = 200)
ComplexHeatmap::Heatmap(ccd_lists,
                        row_order = unlist(clustrow),
                        km = 5,
...
apply(chem_props_scaled, MARGIN = 2, scale) %>%
  ComplexHeatmap::Heatmap(row_order = unlist(clustrow),
                          cluster_columns = FALSE,
                          row_split = enframe(clustrow) %>%
                            unnest(value) %>% pull(name),
                          row_gap = unit(4, "mm"))

apply(this_list %>% select(where(is.numeric)) %>% as.matrix(),
      MARGIN = 2, scale) %>%
  ComplexHeatmap::Heatmap(row_order = unlist(clustrow),
                          cluster_columns = FALSE,
                          row_split = enframe(clustrow) %>%
...
cluster_df %>% inner_join(fittable_kelim %>%
                            rename("kelim_consensus" = "kelim")) %>%
  mutate(kelim_diff = abs(kelim_consensus - kelim)) %>% 
  arrange(kelim_diff) %>% 
  pull(DTXSID) -> kelim_diff_order

ComplexHeatmap::Heatmap(lv4_mat, name = "RMSLE",
                        show_row_names = TRUE,
                        show_column_dend = FALSE,
                        row_names_gp = grid::gpar(fontsize = 7),
                        row_order = match(unique(kelim_diff_order)
                                          , rownames(lv4_mat)))
