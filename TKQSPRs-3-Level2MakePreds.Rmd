...
```
```{r cvt_compare_invitro_outsidedoa, eval = TRUE}
clear_httk()
chem.physical_and_invitro.data <- add_chemtable(
  CvT.chems.nona,
  current.table=chem.physical_and_invitro.data,
...
print(paste("HTTK parameters for",length(unique(c(
  ...
  get_cheminfo(model="gas_pbtk")))),
            "chemicals for PBTK"))
level2tab.invitro2.list <- makeCvTpreds(CvT.data.outsidedoa,"HTTK-InVitro-ODOA")
level2tab.invitro2 <- level2tab.invitro.list$cvt
level2tab.invitro2.stats <- level2tab.invitro.list$stats
level3tab.invitro2 <- maketkstatpreds(CvT.data.outsidedoa,fittable,"HTTK-InVitro-ODOA")
```
