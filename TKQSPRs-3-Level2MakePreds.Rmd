...
### Turned off because of different approach
```{r leave_opera_only_in}
CvT.chems.outsidedoa <- NULL
```

```{r separate_out_opera_only, eval=FALSE}
...
```{r remove_missing_predictions, eval = FALSE}
CvT.chems.nona <- CvT.chems.nobad
...
CvT.data <- subset(CvT.data,DTXSID %in% unique(CvT.data$DTXSID)[1:5])
...
invitro.subset <- CvT.chems.nona[invitro.subset.rows,]

set.seed(123456)
for (i in 1:NUM.YRANDOM) # Ten sets of parameters
{
  yrandom.ids <- sample(1:dim(invitro.subset)[1],dim(invitro.subset)[1])
  # Make sure we don't get the right values:
  accidentally.right <- yrandom.ids[yrandom.ids == 1:dim(invitro.subset)[1]]
  ...
  CvT.chems.nona[invitro.subset.rows,
    ...
    CvT.chems.nona[invitro.subset.rows[yrandom.ids], "Human.Clint.httk"]
  CvT.chems.nona[invitro.subset.rows,
                 paste("Human.Fup.YRandom",i,sep="")] <- 
    CvT.chems.nona[invitro.subset.rows[yrandom.ids], "Human.Fup.httk"]
}
```
