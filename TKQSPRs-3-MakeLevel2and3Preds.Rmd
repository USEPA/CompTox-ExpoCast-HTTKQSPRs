...
for (this.dtxsid in training.set.table$DTXSID)
  for (this.pred in colnames(training.set.table))
    if (this.pred != "DTXSID")
      if (training.set.table[training.set.table$DTXSID== this.dtxsid, 
                            this.pred] == "Possible")
      {
        CvT.chems.notraining[CvT.chems.notraining$DTXSID==this.dtxsid,
          this.pred]<-NA
      }
