...
for (this.qspr in names(level2tab.rmsle))
{
  these.rmsles <- level2tab.rmsle.early[[this.qspr]][
    names(level2tab.rmsle.early[[this.qspr]]) %in% invitro.chems]
  this.count <- length(these.rmsles)
  this.mean <- signif(mean(unlist(these.rmsles), na.rm=TRUE), 3)
  stats.table["RMSLE Early (In Vitro)",this.qspr] <- paste(
    this.mean, " (",
    this.count, ")",
    sep="")
  these.rmsles <- level2tab.rmsle.early[[this.qspr]][
  ...
  these.rmsles <- level2tab.rmsle.late[[this.qspr]][
    names(level2tab.rmsle.late[[this.qspr]]) %in% invitro.chems]
  this.count <- length(these.rmsles)
  this.mean <- signif(mean(unlist(these.rmsles), na.rm=TRUE), 3)
  stats.table["RMSLE Late (In Vitro)",this.qspr] <- paste(
    this.mean, " (",
    this.count, ")",
    sep="")
these.rmsles <- level2tab.rmsle.late[[this.qspr]][
