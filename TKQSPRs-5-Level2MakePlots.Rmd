---
title: "TK QSPRs: Make Level 2 Plots"
...
FigLev2AUC_facet <- ggplot(data=subset(level2tab.stats,
  ...
  geom_point(size=2,
             alpha=0.25,
             aes(x=AUC.pred,y=AUC.obs,shape=QSPR,color=QSPR))+
...
FigLev2Cmax_facet <- ggplot(data=subset(level2tab.stats,
  ...
  geom_point(size=2,
             alpha=0.25,
             aes(x=Cmax.pred,y=Cmax.obs,shape=QSPR,color=QSPR))+
...
firstpreds <- c("In Vivo Fits", "HTPBTK-InVitro", "HTPBTK-Consensus")
...
tiff(file=paste("figs/Fig5-Lev2_RMSEbyChem_all",output.prefix,".tiff",sep=""), 
...
#    labRow = FALSE,
    ...
    trace="none",
    col=mypalette)
tiff(paste("figs/Fig6-RMSE-heatmap-",
           output.prefix,
           ".tiff",sep=""),
...
# ylab(bquote(' ')) +
 xlab(bquote('Predicted Concentraion (mg/L)')) +
 ylab(expression(paste("Observed Conc. ",
    italic("In Vivo"),
    " (mg/L)"))) +
 facet_wrap(~QSPR)+
 theme_bw()+
...
print(FigLev2)
ggsave(paste("figs/Fig4-Lev2_Conc_scatter",output.prefix,".tiff",sep=""), width=6, height=6, dpi=300)
```
