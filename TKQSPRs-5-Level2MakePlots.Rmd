...
load("data/level2predswithstats.RData")
    ...
    for (this.species in unique(level2tab$Species))
    {
      ...
      if (dim(this.subset)[1]>0)
      {
        ...
        level2tab.stats[
          ...
          "RMSLE"] <- RMSLE
        FigCvt <- ggplot(data=this.subset,aes(x=Time,y=Conc.obs)) +
          geom_point(size=3,
                     aes(x=Time,y=Conc.obs,
                         colour = factor(Dose),
                         shape = factor(Route)))+
          geom_line(aes(x=Time,y=Conc.pred,
...
#  scale_y_log10(label=scientific_10,limits=c(10^-2,10^4))+
 ...
 # Pradeep NEW coord_cartesian(ylim=c(-30,210))+ ### !!!!!!! "coord" does NOT alter the data if data points are removed. "scale" does
  ...
  annotation_custom(
    tableGrob(stats.table[c("AAFE","RMSLE"),"HTPBTK-InVitro"],
    rows=NULL, theme = ttheme_minimal(base_size = fontsize)),
...
#  scale_y_log10(label=scientific_10,limits=c(10^-2,10^4))+
 ...
 # Pradeep NEW coord_cartesian(ylim=c(-30,210))+ ### !!!!!!! "coord" does NOT alter the data if data points are removed. "scale" does
  ...
  annotation_custom(
    ...
    xmin=0,xmax=2.3+xshift,ymin=(texty-1),ymax=texty) +
  annotation_custom(
    tableGrob(stats.table[c("AAFE.late","RMSLE.late"),"HTPBTK-ADmet"],
    rows=NULL, theme = ttheme_minimal(base_size = 14)),
    xmin=0,xmax=4.3+xshift,ymin=(texty-1),ymax=texty) +
  annotation_custom(
    tableGrob(stats.table[c("AAFE.late","RMSLE.late"),"HTPBTK-Dawson"],
    rows=NULL, theme = ttheme_minimal(base_size = 14)),
    xmin=0,xmax=6.3+xshift,ymin=(texty-1),ymax=texty)+
  annotation_custom(
    tableGrob(stats.table[c("AAFE.late","RMSLE.late"),"HTPBTK-InVitro"],
    rows=NULL, theme = ttheme_minimal(base_size = 14)),
    xmin=0,xmax=8.3+xshift,ymin=(texty-1),ymax=texty)+
  annotation_custom(
    tableGrob(stats.table[c("AAFE.late","RMSLE.late"),"HTPBTK-OPERA"],
    rows=NULL, theme = ttheme_minimal(base_size = 14)),
    xmin=0,xmax=10.3+xshift,ymin=(texty-1),ymax=texty)+
  annotation_custom(
    tableGrob(stats.table[c("AAFE.late","RMSLE.late"),"HTPBTK-Pradeep"],
    rows=NULL, theme = ttheme_minimal(base_size = 14)),
    xmin=0,xmax=12.3+xshift,ymin=(texty-1),ymax=texty)+
  annotation_custom(
    tableGrob(stats.table[c("AAFE.late","RMSLE.late"),"HTPBTK-YRandom"],
    rows=NULL, theme = ttheme_minimal(base_size = 14)),
    xmin=0,xmax=14.3+xshift,ymin=(texty-1),ymax=texty)+
  theme( text  = element_text(size=20),
  legend.key.height=unit(.5,"line"),
...
for (this.chemical in unique(level2tab$Compound))
{
    ...
    if (regexpr("Caco2",this.qspr)==-1)
  {
    ...
    if (dim(this.subset)[1]>0)
    {
      ...
      this.row$RMSLE <- calc_RMSLE(this.subset)
      this.subset <- subset(level2tab.early,
                            QSPR==this.qspr & Compound==this.chemical)
      this.row$RMSLE.early <- calc_RMSLE(this.subset)
      this.subset <- subset(level2tab.late,
                            QSPR==this.qspr & Compound==this.chemical)
      this.row$RMSLE.late <- calc_RMSLE(this.subset)
...
this.qspr <- "HTPBTK-InVitro-Measured"
stats.table["RMSLE.bychem",this.qspr] <- 
  signif(median(subset(meassub,QSPR=="HTPBTK-InVitro")$RMSLE),3)
stats.table["RMSLE.bychem.early",this.qspr] <-
  signif(median(subset(meassub,QSPR=="HTPBTK-InVitro")$RMSLE.early),3)
stats.table["RMSLE.bychem.late",this.qspr] <-
  signif(median(subset(meassub,QSPR=="HTPBTK-InVitro")$RMSLE.late),3)

write.csv(stats.table,file="tables/Table-Level2Stats.txt",row.names=TRUE)
...
for (this.cas in unique(level2tab$CAS))
{
  ...
  for (this.source in unique(this.subset1$Source))
  {
    ...
    for (this.time in unique(this.subset2$Time))
    {
      ...
      this.subset3.invitro <- subset(this.subset3, QSPR=="HTPBTK-InVitro")
      this.subset3.admet <- subset(this.subset3, QSPR=="HTPBTK-ADmet")
...
qsprbychem.table2 <- apply(qsprbychem.table2[,2:dim(qsprbychem.table2)[2]],2,function(x) signif(as.numeric(x),3))
...
kable(qsprbychem.table2)
```

```{r make_rmse_heatmap, eval = TRUE}
  mypalette<-brewer.pal(9,"PuRd")
  heatmap.2(t(subset(qsprbychem.table2,
                     apply(qsprbychem.table2,1,function(x)
