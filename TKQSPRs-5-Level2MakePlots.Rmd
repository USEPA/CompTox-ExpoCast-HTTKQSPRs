...
for (this.chem in unique(level2tab$DTXSID))
  for (this.QSPR in unique(level2tab$QSPR))
    ...
    for (this.species in unique(level2tab$Species))
    {
      ...
      {
        residuals <- log10(this.subset$Conc.obs / 
                                      this.subset$Conc.pred)
        residuals <- residuals[!is.infinite(residuals)]
        residuals <- residuals[!is.nan(residuals)]
        RMSLE <- signif((mean(residuals^2,
                       na.rm=TRUE))^(1/2),3)
