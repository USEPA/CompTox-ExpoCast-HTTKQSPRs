...
# Pull in the consensus Vd's so we can calculated clearance:
consensus.results <- level3tab.old[["HTPBTK-Consensus"]]
for (this.chem in level3tab.arnot1$DTXSID)
  if (this.chem %in% consensus.results$DTXSID)
{
  level3tab.arnot1[level3tab.arnot1$DTXSID==this.chem,"Vd.pred"] <- 
    consensus.results[consensus.results$DTXSID==this.chem,"Vd.pred"]
}
...
# Calculate the clearance using the consensus Vd
level3tab.arnot1$cl.pred <- level3tab.arnot1$Vd.pred * log(2) / level3tab.arnot1$thalf.pred 
