...
this.subset <- subset(level3tab,QSPR=="HTPBTK-InVitro" & CAS %in% httk.both)
this.fit <- lm(data=this.subset,log10(thalf.obs)~log10(thalf.pred))
this.row <- data.frame(
  QSPR=paste("HTPBTK-InVitro","Measured",sep="-"),
  R2=signif(summary(this.fit)$adj.r.squared,3),
  RMSE=signif(sqrt(mean(summary(this.fit)$residuals^2)),3)
  )
halflife.stats.table <- rbind(halflife.stats.table,this.row)
kable(halflife.stats.table)
write.csv(halflife.stats.table,file="tables/SupTable-halflife-stats.txt",row.names=FALSE)
```
...
write.csv(vd.stats.table,file="tables/SupTable-VD-stats.txt",row.names=FALSE)
```
...
write.csv(cl.stats.table,file="tables/SupTable-Cl-stats.txt",row.names=FALSE)
```
...
print(FigLev3a)
ggsave(paste("figs/FigLev3a_thalf_obsVpred_",output.prefix,".tiff",sep=""), width=6, height=4, dpi=300)
```
...
print(FigLev3b) 
ggsave(paste("figs/FigLev3b_thalf_boxplot_",output.prefix,".tiff",sep=""), width=6, height=4, dpi=300)
```
...
FigLev3c <- ggplot(data=subset(level3tab,!(QSPR %in% c("IFS-QSPR","QSPRINS")))) +
  ...
  theme( text  = element_text(size=20),
         ...
         axis.text.x = element_text(size=10))
print(FigLev3c)
ggsave(paste("figs/FigLev3c_Vd_obsVpred_",output.prefix,".tiff",sep=""), 
...
#  scale_y_log10(breaks=c(10^-2,10^-1,1,3,10,10^2,10^3),label=scientific_10)+
  ...
  theme(axis.text.x = element_text(angle = 60, vjust = 0.75, hjust=0.75))
print(FigLev3d)  
ggsave(paste("figs/FigLev3d_Vd_boxplot_",output.prefix,".tiff",sep=""), 
       width=6, height=4, dpi=300)
```                 
...
FigLev3e <- ggplot(data=level3tab) +
  ...
  theme( text  = element_text(size=20),
         ...
         axis.text.x = element_text(size=10))
print(FigLev3e)
ggsave(paste("figs/FigLev3e_Cltot_obsVpred_",output.prefix,".tiff",sep=""), 
       width=6, height=4, dpi=300)
```
...
#  scale_y_log10(breaks=c(10^-2,10^-1,1,3,10,10^2,10^3),label=scientific_10)+
  ...
  theme(axis.text.x = element_text(angle = 60, vjust = 0.75, hjust=0.75))
print(FigLev3f)
ggsave(paste("figs/FigLev3f_Cltot_boxplot_",output.prefix,".tiff",sep=""), 
       width=6, height=4, dpi=300)
``` 
...
write.csv(lev3suptable,row.names=FALSE,file="tables/SupTable-Level3.txt")
