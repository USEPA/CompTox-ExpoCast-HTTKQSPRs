...
ccd_lists <- read_csv("ccd_data/CCD-Batch-Search_2024-07-11_02_19_19.csv")
...
this_list <- list()
null.parms <- parameterize_gas_pbtk(chem.name="bisphenola")
null.parms[] <- NA
for (i in 1:length(pull(chem_props, DTXSID))) if(pull(chem_props,
                          DTXSID)[i] %in% 
                    get_cheminfo(info="DTXSID",model="gas_pbtk"))
  {
  this_list <- bind_rows(this_list, parameterize_gas_pbtk(dtxsid = pull(chem_props,
                                                      DTXSID)[i]) %>%
    as.data.frame())
} else {
   this_list <- bind_rows(this_list, null.parms) 
  }
...
my_smiles <- read_csv("ccd_data/CCD-Batch-Search_2024-07-16_08_42_30.csv")
