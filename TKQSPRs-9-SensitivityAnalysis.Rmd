---
title: "HTTK QSPRs: Sensitivity Analysis"
...
```{r create_ensemble_preds, eval=TRUE}
clint.cols <- colnames(CvT.chems.nobad)[
  regexpr("Clint",colnames(CvT.chems.nobad)) != -1 & # All Clintpredictions
  regexpr("httk",colnames(CvT.chems.nobad)) == -1]   # Except in vitro datas
...
fup.cols <- colnames(CvT.chems.nobad)[
  regexpr("Fup",colnames(CvT.chems.nobad))!=-1 &   # All Fup predictions
  regexpr("httk",colnames(CvT.chems.nobad)) == -1] # Except in vitro data
...
## Make a version of the prediction matrix where we've replaced QSPR-chemical combinations with no predictions with the ensemble predictions:
...
```{r cvt_compare_ensemble, eval = TRUE}
this.label <- "HTPBTK-Ensemble" 
...
the ensemble predicted values changes the values. 
...
# HTPBTK-Ensemble predictions found in level3tab$`HTPBTK-Ensemble`
# Get names for fittable
fit_names <- names(fittable)
# For Vdist
fittable_Vd <- inner_join(fittable, level3tab$`HTPBTK-Ensemble`,
                          by = c(
                            "DTXSID",
...
# For kelim
fittable_kelim <- inner_join(fittable, level3tab$`HTPBTK-Ensemble`,
                          by = c(
                            "DTXSID",
                            "PREFERRED_NAME" = "Compound",
                            "CAS",
...
# For Fgutabs
fittable_Fgutabs <- inner_join(fittable, level3tab$`HTPBTK-Ensemble`,
                          by = c(
                            "DTXSID",
                            "PREFERRED_NAME" = "Compound",
                            "CAS",
...
lv4_df <- data.frame(
  QSPR = c(rep("HTPBTK-Ensemble",
               length(unlist(level2tab.rmsle$`HTPBTK-Ensemble`))),
           rep("In Vivo",
  ...
  DTXSID = c(names(unlist(level2tab.rmsle$`HTPBTK-Ensemble`)),
  ...
  RMSLE = c(unlist(level2tab.rmsle$`HTPBTK-Ensemble`),
...
```{r make_substition_plot}
qsprs.order <- c("In Vivo",
                                               "In Vivo-HTTK_Vdist",
                                               "In Vivo-HTTK_Fgutabs",
                                               "In Vivo-HTTK_kelim",
                                               "HTPBTK-Ensemble"
...
RMSLE.table <- data.frame()
for (this.qspr in qsprs.order)
{
  RMSLE.table["RMSLE",this.qspr] <-
    signif(mean(subset(lv4_df, QSPR==this.qspr)$RMSLE,na.rm=TRUE),2)
}
texty <- 2.3
xshift <- 0
stat.labels <- "Mean"

FigSensitivity <- ggplot(data = lv4_df %>%
         mutate(QSPR = factor(QSPR, levels = qsprs.order,
                              ordered=TRUE)),
       aes(x = QSPR,
           y = RMSLE)) +
...
{
  FigSensitivity <- FigSensitivity + annotation_custom(
    tableGrob(RMSLE.table[
      "RMSLE",qsprs.order[i]],
      rows=NULL, 
...
print(FigSensitivity)
ggsave(filename = "figs/Fig9-SensitivityAnalysis.png",
       height = 5,
       width = 7)
```
...
the RMSLE when Fgutabs, kelim, or Vdist values from Ensemble 
...
and another cluster (5) that are fairly improved in the ensemble model.

```{r clustering_analysis, eval=FALSE}
...
cluster_df %>%
  select(DTXSID, Clust) %>%
  inner_join(level3tab$`HTPBTK-Ensemble` %>% select(-Compound)) %>%
...
ccd_lists <- ccd_lists <- read_csv("ccd_data/CCD-Batch-Search_2024-07-11_02_19_19.csv")
...
start at the In Vivo value and head towards the Ensemble value in 10% increments.


```{r sensitivity_analysis, eval = TRUE}
# For Vdist
fittable_Vdist <- inner_join(fittable, level3tab$`HTPBTK-Ensemble`,
                          by = c(
                            "DTXSID",
                            "PREFERRED_NAME" = "Compound",
                            "CAS",
...
that is not the Ensemble Value.
...
CvT.chems.nona.kelim <- kelim_substitution(data = CvT.chems.nona,
                                           invivo = fittable,
                                           Vd_data = level3tab$`HTPBTK-Ensemble`,
