...
for (this.study in unique(CvT.data$Source))
{
  ...
  for (this.chem in unique(this.subset1$CAS))
  {
    ...
    {
      this.subset3 <- subset(this.subset2,Media==this.media)
      this.loq <- min(this.subset3$Value,na.rm=TRUE)
...
Make a version of the prediciton matrix where we've smoothed out missing data:
...
chem.physical_and_invitro.data <- add_chemtable(
  ...
  reference="InVitro",
  overwrite=T)
level2tab.invitro.list <- suppressWarnings(makeCvTpreds("HTTK-InVitro"))
...
Lev2.auc <- ggplot(data=level2tab.stats, aes(x=QSAR, y=AUC.RPE)) + 
#  geom_hline(yintercept=1,linetype = "dashed",color="blue")+
#  geom_hline(yintercept=3,linetype = "dashed",color="blue")+
#  geom_hline(yintercept=10,linetype = "dashed",color="blue")+
  geom_boxplot(outlier.colour="red",
