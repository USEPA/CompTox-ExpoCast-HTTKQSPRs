...
httk.data[httk.data$Human.Fup == 0,"Human.Fup"] <- NA
```
...
for (this.qspr1 in unique(level1tab$QSPR))
{
  ...
  for (this.qspr2 in unique(level1tab$QSPR))
  {
    ...
    ks.tests.clint[this.qspr1,this.qspr2] <- suppressWarnings(
      ks.test(set1,set2)$p.value)
    set1 <- this.subset$Human.fup.FE
    set2 <- subset(level1tab,QSPR==this.qspr2)$Human.fup.FE
    ks.tests.fup[this.qspr1,this.qspr2] <- suppressWarnings(
      ks.test(set1,set2)$p.value)
  }
}
...
FigLev1a_facet <- ggplot(data=level1tab) +
  geom_point(size=2,aes(x=Human.Clint.pred+10^-4,y=Human.Clint.httk+10^-4,shape=QSPR,color=QSPR))+
...
CvT.chems.nobad$Human.Clint.Consensus <- apply(
...
CvT.chems.nobad$Human.Fup.Consensus <- apply(
          ...
          for (this.time in unique(this.subset4$Time))
          {
            ...
            for (this.obs in unique(this.subset5$Conc.obs))
            {
              ...
              mean.pred <- mean(subset(this.subset6,
                !(QSPR %in% c("FitsToData","YRandom")))$Conc.pred,na.rm=TRUE)
...
level3.arnot <- read_excel("TK_QSPR_evaluation_set_HumanQSPRHLPredictions.xlsx")
...
level3tab.arnot1$QSPR <- "QSPRINS"
level3tab.arnot2$QSPR <- "IFS-QSPR"
