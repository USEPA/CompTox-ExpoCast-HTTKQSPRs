...
CvT.chems.nona[,"Human.Clint.YRandom"] <- 
  CvT.chems.nona[yrandom.ids, "Human.Clint.httk"]
CvT.chems.nona[,"Human.Fup.YRandom"] <- 
...
chem.physical_and_invitro.data <- add_chemtable(
  ...
  data.list=list(
    ...
    DTXSID="DTXSID",
    CAS="CASRN",
    Funbound.plasma="Human.Fup.YRandom",
    Clint="Human.Clint.YRandom"),
  species="Human",
  reference="YRandom",
  overwrite=T)
...
level2tab.yrandom <- level2tab.yrandom.list$cvt
level2tab.yrandom.stats <- level2tab.yrandom.list$stats
level3tab.yrandom <- maketkstatpreds(CvT.data,fittable,"HTTK-YRandom")
```
          ...
          for (this.time in unique(this.subset4$Time))
          {
            ...
            for (this.obs in unique(this.subset5$Conc.obs))
            {
              ...
              mean.pred <- mean(subset(this.subset6,
                !(QSAR %in% c("FitsToData","YRandom")))$Conc.pred,na.rm=TRUE)
...
for (this.qsar in unique(level2tab$QSAR))
{
#Absolute Average Fold Error (AAFE) :  10^((1/n)*sum(abs(FE)))  ### use this. It's good to see fold error
  AAFE[[this.qsar]] <- 10^(mean(level2tab$AbsFE[
    level2tab$QSAR==this.qsar],na.rm=TRUE))
  AAFE.early[[this.qsar]] <- 10^(mean(level2tab.early$AbsFE[
    level2tab.early$QSAR==this.qsar],na.rm=TRUE))
  AAFE.late[[this.qsar]] <- 10^(mean(level2tab.late$AbsFE[
    level2tab.late$QSAR==this.qsar],na.rm=TRUE))
#RMSLE sqrt(mean(log10(Xpred+1)-log10(Xobs+1))2)  ### use this
  RMSLE[[this.qsar]] <- sqrt(mean((log10(level2tab$Conc.pred[
    level2tab$QSAR==this.qsar] /
    level2tab$Conc.obs[level2tab$QSAR==this.qsar]))^2,na.rm=TRUE))
  RMSLE.early[[this.qsar]] <-sqrt(mean((log10(level2tab.early$Conc.pred[
    level2tab.early$QSAR==this.qsar]/
    level2tab.early$Conc.obs[level2tab.early$QSAR==this.qsar]))^2,na.rm=TRUE))
  RMSLE.late[[this.qsar]] <-sqrt(mean((log10(level2tab.late$Conc.pred[
    level2tab.late$QSAR==this.qsar]/
    level2tab.late$Conc.obs[level2tab.late$QSAR==this.qsar]))^2,na.rm=TRUE))
  MRPE[[this.qsar]] <- median(subset(level2tab,QSAR==this.qsar)$RPE)
  MRPE.early[[this.qsar]] <- median(subset(level2tab.early,QSAR==this.qsar)$RPE)
  MRPE.late[[this.qsar]] <- median(subset(level2tab.late,QSAR==this.qsar)$RPE)
}

stats.table <- data.frame()
for (this.qsar in names(RMSLE))
{
  stats.table["AAFE",this.qsar] <- round(AAFE[[this.qsar]],2)
  stats.table["RMSLE",this.qsar] <- round(RMSLE[[this.qsar]],2)
  stats.table["MRPE",this.qsar] <- round(MRPE[[this.qsar]],2)
  stats.table["RMSLE.early",this.qsar] <- round(RMSLE.early[[this.qsar]],2)
  stats.table["RMSLE.late",this.qsar] <- round(RMSLE.late[[this.qsar]],2)
  stats.table["AAFE.early",this.qsar] <- round(AAFE.early[[this.qsar]],2)
  stats.table["AAFE.late",this.qsar] <- round(AAFE.late[[this.qsar]],2)
  stats.table["MRPE.early",this.qsar] <- round(MRPE.early[[this.qsar]],2)
  stats.table["MRPE.late",this.qsar] <- round(MRPE.late[[this.qsar]],2)
}
...
#  scale_y_log10(label=scientific_10,limits=c(10^-2,10^4))+
 ...
 # Pradeep NEW coord_cartesian(ylim=c(-30,210))+ ### !!!!!!! "coord" does NOT alter the data if data points are removed. "scale" does
  ...
  theme_bw()+
  annotation_custom(
    tableGrob(c("RMSLE","AAFE"), theme = ttheme_minimal(base_size = 12)),
    xmin=0,xmax=1.2+xshift,ymin=(texty-1),ymax=texty)+
  annotation_custom(
    tableGrob(stats.table[c("RMSLE","AAFE"),"FitsToData"],
    rows=NULL, theme = ttheme_minimal(base_size = 14)),
    xmin=0,xmax=2.3+xshift,ymin=(texty-1),ymax=texty) +
  annotation_custom(
    tableGrob(stats.table[c("RMSLE","AAFE"),"HTTK-ADmet"],
    rows=NULL, theme = ttheme_minimal(base_size = 14)),
    xmin=0,xmax=4.3+xshift,ymin=(texty-1),ymax=texty) +
  annotation_custom(
    tableGrob(stats.table[c("RMSLE","AAFE"),"HTTK-Dawson"],
    rows=NULL, theme = ttheme_minimal(base_size = 14)),
    xmin=0,xmax=6.3+xshift,ymin=(texty-1),ymax=texty)+
  annotation_custom(
    tableGrob(stats.table[c("RMSLE","AAFE"),"HTTK-InVitro"],
    rows=NULL, theme = ttheme_minimal(base_size = 14)),
    xmin=0,xmax=8.3+xshift,ymin=(texty-1),ymax=texty)+
  annotation_custom(
    tableGrob(stats.table[c("RMSLE","AAFE"),"HTTK-OPERA"],
    rows=NULL, theme = ttheme_minimal(base_size = 14)),
    xmin=0,xmax=10.3+xshift,ymin=(texty-1),ymax=texty)+
  annotation_custom(
    tableGrob(stats.table[c("RMSLE","AAFE"),"HTTK-Pradeep"],
    rows=NULL, theme = ttheme_minimal(base_size = 14)),
    xmin=0,xmax=12.3+xshift,ymin=(texty-1),ymax=texty)+
  annotation_custom(
    tableGrob(stats.table[c("RMSLE","AAFE"),"HTTK-YRandom"],
    rows=NULL, theme = ttheme_minimal(base_size = 14)),
    xmin=0,xmax=14.3+xshift,ymin=(texty-1),ymax=texty)+  
  theme( text  = element_text(size=20),
  legend.key.height=unit(.5,"line"),
...
texty <- 2.5
xshift <- 0.2
these.stats <- c("RMSLE.early","AAFE.early")
...
#  scale_y_log10(label=scientific_10,limits=c(10^-2,10^4))+
 ...
 # Pradeep NEW coord_cartesian(ylim=c(-30,210))+ ### !!!!!!! "coord" does NOT alter the data if data points are removed. "scale" does
  ...
  annotation_custom(
    ...
    xmin=0,xmax=1.2+xshift,ymin=(texty-1),ymax=texty)+
  annotation_custom(
    tableGrob(stats.table[these.stats,"FitsToData"],
    rows=NULL, theme = ttheme_minimal(base_size = 14)),
    xmin=0,xmax=2.3+xshift,ymin=(texty-1),ymax=texty) +
  annotation_custom(
    tableGrob(stats.table[these.stats,"HTTK-ADmet"],
    rows=NULL, theme = ttheme_minimal(base_size = 14)),
    xmin=0,xmax=4.3+xshift,ymin=(texty-1),ymax=texty) +
  annotation_custom(
    tableGrob(stats.table[these.stats,"HTTK-Dawson"],
    rows=NULL, theme = ttheme_minimal(base_size = 14)),
    xmin=0,xmax=6.3+xshift,ymin=(texty-1),ymax=texty)+
  annotation_custom(
    tableGrob(stats.table[these.stats,"HTTK-InVitro"],
    rows=NULL, theme = ttheme_minimal(base_size = 14)),
    xmin=0,xmax=8.3+xshift,ymin=(texty-1),ymax=texty)+
  annotation_custom(
    tableGrob(stats.table[these.stats,"HTTK-OPERA"],
    rows=NULL, theme = ttheme_minimal(base_size = 14)),
    xmin=0,xmax=10.3+xshift,ymin=(texty-1),ymax=texty)+
  annotation_custom(
    tableGrob(stats.table[these.stats,"HTTK-Pradeep"],
    rows=NULL, theme = ttheme_minimal(base_size = 14)),
    xmin=0,xmax=12.3+xshift,ymin=(texty-1),ymax=texty)+
  annotation_custom(
    tableGrob(stats.table[these.stats,"HTTK-YRandom"],
    rows=NULL, theme = ttheme_minimal(base_size = 14)),
    xmin=0,xmax=14.3+xshift,ymin=(texty-1),ymax=texty)+ 
  theme( text  = element_text(size=20),
  legend.key.height=unit(.5,"line"),
...
texty <- 2.5
xshift <- 0.2
these.stats <- c("RMSLE.late","AAFE.late")
stat.labels <- c("RMSLE","AAFE")
...
qsarbychem.table2 <- apply(qsarbychem.table2[,2:(1+length(QSARs))],2,as.numeric)[,1:7]
rownames(qsarbychem.table2) <- chems
colnames(qsarbychem.table2) <- c("FitsToData","ADmet","Dawson","InVitro","OPERA","Pradeep","Y-Random")
write.csv(qsarbychem.table2,row.names=FALSE,file="SupTable-RMSLEbyChem.txt")
```
...
for (this.qsar in unique(level3tab$QSAR))
{
  this.subset <- subset(level3tab,QSAR==this.qsar)
...
vd.stats.table <- NULL
for (this.qsar in unique(level3tab$QSAR))
if (!(this.qsar %in% c("IFS-QSAR","QSARINS")))
{
  this.subset <- subset(level3tab,QSAR==this.qsar&!is.na(Vd.obs)&!is.na(Vd.pred))
...
FigLev3e <- ggplot(data=level3tab) +
  geom_point(size=3,aes(x=Cl.pred,y=Cl.obs,shape=QSAR,color=QSAR))+
  scale_x_log10(label=scientific_10)+
  scale_y_log10(label=scientific_10)+
  scale_shape_manual(values=c(15, 16, 17, 18, 25,0,1,2,5))+
  geom_abline(intercept = log10(10^(1/2)), slope = 1,linetype="dashed", colour="lightBlue") +
