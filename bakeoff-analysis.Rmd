...
for (this.qsar in unique(level2tab.stats$QSAR))
{
  ...
  for (this.chem in unique(this.subset$Compound))
  {
    this.subset2 <- subset(this.subset,Compound==this.chem)
    this.index <- CvT.chems.stats.heatmap$PREFERRED_NAME==this.chem
    ...
    this.subset3 <- subset(level2tab, QSAR==this.qsar & Compound==this.chem)
...
print(this.chem)
  this.index <- CvT.chems.stats.heatmap$PREFERRED_NAME==this.chem
  v <- CvT.chems.stats.heatmap[this.index,regexpr("AUC.RPE",
  ...
  v <- CvT.chems.stats.heatmap[this.index,regexpr("Cmax.RPE",
    colnames(CvT.chems.stats.heatmap))!=-1]
  print(v)
  if (any(!is.na(v)))
  {
  ...
  if (any(!is.na(v)))
  {
    m <- mean(v,na.rm=TRUE)
    s <- sd(v,na.rm=TRUE)
    CvT.chems.stats.heatmap[this.indexregexpr("RMSLE",
      colnames(CvT.chems.stats.heatmap))!=-1] <- 
      (v - m)/s
  }
}
