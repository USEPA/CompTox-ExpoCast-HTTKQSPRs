...
```{r load_cvt_data_old, eval = FALSE}
...
# Get chemical names, ids', and phys-chem properties:
```{r load_dashboard_info}
...
series_res_set$Time <- series_res_set$conc_time_values.time_h
series_res_set$Value <- series_res_set$conc_time_values.conc
series_res_set$Dose <- series_res_set$studies.dose_level_normalized
series_res_set$Route <- series_res_set$studies.administration_route_normalized
series_res_set$calc_loq <- series_res_set$series.loq_normalized
...
series_res_set$Source <- series_res_set$documents_extraction.pmid
```
...
write.csv(CvT.data,row.names=FALSE,file="SupTable/SupTable-CvTData.txt")
```

# Calculate a limit of quantification (90% of lowest observation)
```{r calc_loq_old, eval = FALSE}
...
```{r merge_curve_fits_old, eval = FALSE}
fittable <- NULL
...
# Calculate halflife:
fittable$halflife <- signif(log(2)/fittable$kelim,3)
# Although CvTdb has expanded, keep focus on human and rat:
...
write.csv(fittable,file="SupTable/SupTable-TKFits.txt",row.names=FALSE)
```
...
fittable <- subset(fittable, !(Model%in% c("model_flat")))
# Don't want data where couldn't estimate Vdist or V1:
fittable <- subset(fittable, !is.na(Vdist) | !is.na(V1))
...
write.csv(sup.table, row.names=FALSE,file="SupTable/SupTable-QSPRPredsandInVitroData.txt")
```
...
FigLev3a <- ggplot(data=level3tab) +
  ...
  theme( text  = element_text(size=20),
         ...
         axis.text.x = element_text(size=10))
print(FigLev3a)
ggsave(paste("figs/FigLev3a_thalf_obsVpred_",output.prefix,".tiff",sep=""), width=4, height=1.7, dpi=300)
```
...
#  scale_y_log10(breaks=c(10^-2,10^-1,1,3,10,10^2,10^3),label=scientific_10)+
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5))
print(FigLev3b) 
ggsave(paste("figs/FigLev3b_thalf_boxplot_",output.prefix,".tiff",sep=""), width=4, height=1.7, dpi=300)
```
...
FigLev3c <- ggplot(data=subset(level3tab,!(QSPR %in% c("IFS-QSPR","QSPRINS")))) +
  ...
  theme( text  = element_text(size=20),
         ...
         axis.text.x = element_text(size=10))
print(FigLev3c)
ggsave(paste("figs/FigLev3c_Vd_obsVpred_",output.prefix,".tiff",sep=""), width=4, height=1.7, dpi=300)
```
...
#  scale_y_log10(breaks=c(10^-2,10^-1,1,3,10,10^2,10^3),label=scientific_10)+
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5))
print(FigLev3d)  
ggsave(paste("figs/FigLev3d_Vd_boxplot_",output.prefix,".tiff",sep=""), width=4, height=1.7, dpi=300)
```                 
...
FigLev3e <- ggplot(data=level3tab) +
  ...
  theme( text  = element_text(size=20),
         ...
         axis.text.x = element_text(size=10))
print(FigLev3e)
ggsave(paste("figs/FigLev3e_Cltot_obsVpred_",output.prefix,".tiff",sep=""), width=4, height=1.7, dpi=300)
```
...
#  scale_y_log10(breaks=c(10^-2,10^-1,1,3,10,10^2,10^3),label=scientific_10)+
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5))
print(FigLev3f)
ggsave(paste("figs/FigLev3f_Cltot_boxplot_",output.prefix,".tiff",sep=""), width=4, height=1.7, dpi=300)
``` 
...
write.csv(lev3suptable,row.names=FALSE,file="SupTable/SupTable-Level3.txt")
